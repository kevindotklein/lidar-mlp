...
namespace CSV {
    ...
    void printClassBalance(const std::vector<std::vector<double>>& features, 
        ...
        data << "----------- Class Balance:  -----------\n";
        data << "Amostras totais: " << total << "\n";
        data << "Positiva (Colisao): " << positive << " (" << pos_percent << "%)\n";
        data << "Negativa (Sem Colisao): " << negative << " (" << neg_percent << "%)\n";
        data << "---------------------------------------------------\n";
                                    ...
                                    [](const std::string& l) {return isCollisionLabel(l) ? 1 : 0;})
    {
        ...
        data << "----------------- dados de treino -------------------" << std::endl;
        data << "Features: " << training_features.size() << " samples, "
            << (training_features.empty() ? 0 : training_features[0].size()) << " features cada\n";
        data << "Labels: " << training_labels.size() << "\n";
        data << "IDs: " << training_ids.size() << "\n";
        data << "----------------- dados de teste -------------------" << std::endl;
        data << "Features: " << test_features.size() << " samples, "
            << (test_features.empty() ? 0 : test_features[0].size()) << " features cada\n";
